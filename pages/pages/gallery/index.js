...
import backPic from "../../assets/img/bg/gallery.jpg";
...
const Attribute = ({ name, searchAttr, toggleAttr }) => {
  ...
  const attrSelected = (attr) => {
    const index = searchAttr.findIndex(
      (item) => item.trait_type === name && item.value === attr
    );
    return index !== -1;
  };
  ...
  const selectedCount = values.filter((value) => {
    return (
      searchAttr.findIndex(
        (item) => item.trait_type === name && item.value === value
      ) !== -1
    );
  }).length;
  ...
  return (
    <div className="my-2">
      <div
        className="flex justify-between items-center cursor-pointer pb-3 px-1"
        onClick={() => setOpen((prev) => !prev)}
      >
        <div className="flex justify-center items-center">
          <div className="attrImg mr-2">
            <Image alt="pic" src={img.default}></Image>
          </div>
          <div className="attrName">{name}</div>

          {selectedCount > 0 ? (
            <div className="attrBadge text-base flex items-center justify-center ml-2">
              {selectedCount}
            </div>
          ) : null}
        </div>
        ...
        <div className="arrowIcon">
          <Image alt="pic" src={open ? minusIcon : plusIcon}></Image>
        </div>
      </div>

      <div
        className={`lg:max-h-80 lg:overflow-y-auto relative attrContent px-2 ${
          open ? "active" : ""
        }`}
      >
...
export const Gallery = () => {
  ...
  const getFilteredData = (data) => {
    return data.filter((item) => {
      ...
      if (
        searchTokenId !== "" &&
        Number(item.token_id) !== Number(searchTokenId)
      )
        return false;
      ...
      for (let i = 0; i < searchAttr.length; i++) {
        ...
        const index = metadata.attributes.findIndex(
          (attr) =>
            attr.trait_type === temp.trait_type && attr.value === temp.value
        );
  ...
  const toggleAttr = (name, attr) => {
    ...
    const index = newSearchAttr.findIndex(
      (item) => item.trait_type === name && item.value === attr
    );

    if (index === -1) newSearchAttr.push({ trait_type: name, value: attr });
    else newSearchAttr.splice(index, 1);

    setSearchAttr(newSearchAttr);
  };

  const getImageUrl = (item) => {
    let imageUrl = JSON.parse(item.metadata).image;
    imageUrl = imageUrl.replace(
      "ipfs://",
      "https://nounpunks.mypinata.cloud/ipfs/"
    );
    return imageUrl;
  };
  ...
  const detectScroll = () => {
    if (
      ...
      (document.body.offsetHeight * 90) / 100
    ) {
      ...
      <div className="container">
        <div className={`gallery__filter ${showFilter ? "active" : ""}`}>
          ...
          <div className="gallery__filter__search">
            ...
            />
            <Image src={searchIcon} alt="pic"></Image>
          </div>
          ...
          <div
            className="gallery__filter__collapse"
          ...
          <div className="gallery__content__wrapper relative">
            {dataArray.length > 0 &&
              dataArray.map((item, index) => (
                <div
                  className="item p-2 flex flex-col relative"
                  key={`galleryitem__${index}`}
                  onClick={() => onClickImage(item)}
                >
                  <div className="item__title w-full">
                    superBLOX #{item.token_id}
                  </div>

                  <ProgressiveImg
                    placeholderSrc={placeholderSrc}
                    src={`https://nounpunks.mypinata.cloud/ipfs/QmQUme3g63psPnu8cuvhaS9NAFoPfoyMvV5zyYBDCMwfYM/${item.token_id}.jpg`}
                    alt="pic"
                  />
                </div>
              ))}
            ...
            {allNFTData.length > 0 && !dataArray.length ? (
              <div className="flex flex-col w-full justify-center items-center mt-12">
                ...
                <button
                  className="clearFilterBtn p-4 mt-4"
